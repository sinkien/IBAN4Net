...
namespace SinKien.IBAN4Net.NetStandard.Tests.Net45
{
	...
	public class IbanUtilsTest
	{
		...
		[TestMethod]
		public void IsFromSEPACountryShouldReturnFalseIfItIsNotFromMemberCountry()
		{
			string iban = "AL35202111090000000001234567"; // Albania is not SEPA member            
			Assert.IsFalse(IbanUtils.IsFromSEPACountry(iban));
		}

		[TestMethod]
		public void IsFromSEPACountryShouldReturnTrueIfItIsFromMemberCountry()
		{
			string iban = "CZ5508000000001234567899"; // Czech Republic is SEPA, but not eurozone member            
			Assert.IsTrue(IbanUtils.IsFromSEPACountry(iban));
		}

		[TestMethod]
		public void IsFromEurozoneSEPACountryShouldReturnFalseIfItIsNotFromEurozoneMemberCountry()
		{
			string iban = "CZ5508000000001234567899"; // Czech Republic is SEPA, but not eurozone member            
			Assert.IsFalse(IbanUtils.IsFromEurozoneSEPACountry(iban));
		}

		[TestMethod]
		public void IsFromEurozoneSEPACountryShouldReturnTrueIfItIsFromEurozoneMemberCountry()
		{
			string iban = "FI1410093000123458"; // Finland is SEPA and Eurozone member            
			Assert.IsTrue(IbanUtils.IsFromEurozoneSEPACountry(iban));
		}
